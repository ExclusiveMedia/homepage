...
const nav = [['Home','/'],['Scorecard','/scorecard/'],['Audit','/audit/'],['Services','/services/'],['Industries','/industries/'],['Proof','/case-studies/'],['FAQ','/faq/'],['Apply','/contact/']];

function html(title, body, bullets){return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
<body>
  <header class="top"><div class="brand"><strong>Exclusive Media</strong><span>Digital Growth Architecture</span></div><nav>${nav.map(([l,h])=>`<a href="${h}">${l}</a>`).join('')}</nav></header>
  <main class="shell">
    <p class="alert">Live Status: Limited Diagnostic Growth Audit Slots Remaining</p>
    <section class="hero">
      <div><p class="eyebrow">One connected growth system</p><h1>${title}</h1><p>${body}</p><a class="cta" href="/scorecard/">Calculate My Revenue Leaks</a><a class="cta secondary" href="/contact/">Apply for Audit</a></div>
...
for (const [file,title,body,bullets] of pages){ mkdirSync(dirname(file), { recursive: true }); writeFileSync(file, html(title,body,bullets)); }
